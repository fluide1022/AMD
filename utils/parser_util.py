...
# Load parameter function generated by custom background call
def custom_parse_and_load_from_model(parser):
	# args according to the loaded model
	# do not try to specify them from cmd line since they will be overwritten
	add_data_options(parser)
	add_model_options(parser)
...
def add_data_options(parser):
	...
	group.add_argument("--dataset", default='humanml', choices=['humanml', 'kit', 'humanact12', 'uestc', 'autoreg'], type=str,
...
# Custom sampling settings: only allow background code calls
# so the model_path will be specified by the code and does not require explicit console input
def add_custom_sampling_options(parser, model_path):
	group = parser.add_argument_group('custom_sampling')
	group.add_argument("--model_path", default=model_path, type=str,
										 help="Path to model####.pt file to be sampled.")
	group.add_argument("--output_dir", default='', type=str,
										 help="Path to results dir (auto created by the script). "
													"If empty, will create dir in parallel to checkpoint.")
...
# The configuration function generated by the custom background call
def custom_generate_args(model_path):
	parser = ArgumentParser()
	# args specified by the user: (all other will be loaded from the model)
	add_base_options(parser)
	add_custom_sampling_options(parser, model_path)
	add_generate_options(parser)
	return custom_parse_and_load_from_model(parser)
...
# Customized background call edited configuration function
def custom_edit_args(model_path):
	parser = ArgumentParser()
	# args specified by the user: (all other will be loaded from the model)
	add_base_options(parser)
	add_custom_sampling_options(parser, model_path)
