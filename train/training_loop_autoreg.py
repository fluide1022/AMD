...
from eval import eval, eval_humanact12_uestc
...
class TrainLoopAutoReg:
    ...
    def run_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for motion_0, cond_0, motion_1, cond_1 in tqdm(self.data):
                if not (not self.lr_anneal_steps or self.step + self.resume_step < self.lr_anneal_steps):
                    break

                motion_0 = motion_0.to(self.device)
                cond_0['y'] = {key: val.to(self.device) if torch.is_tensor(val) else val for key, val in cond_0['y'].items()}

                motion_1 = motion_1.to(self.device)
                cond_1['y'] = {key: val.to(self.device) if torch.is_tensor(val) else val for key, val in cond_1['y'].items()}

                self.run_step(motion_0, cond_0, motion_1, cond_1)
    ...
    def run_step(self, batch_0, cond_0, batch_1, cond_1):
        self.forward_backward(batch_0, cond_0, batch_1, cond_1)
        self.mp_trainer.optimize(self.opt)
        self._anneal_lr()
        # print('lr: ', self.opt.param_groups[0]['lr'])
        self.log_step()

    def forward_backward(self, batch_0, cond_0, batch_1, cond_1):
        self.mp_trainer.zero_grad()
        for i in range(0, batch_0.shape[0], self.microbatch):
            ...
            loss0, motion_0_out = compute_losses_0()
            # print(loss0['loss'])

            # forward 1
            micro_cond_1['y']['motion_prev'] = motion_0_out
